...
def simular_GPU() -> tuple:
    '''
        Simula o uso e a temperatura da GPU, com valores entre 40 á 90 na porcentagem e na temperatura 45 á 90.

        params:
            - None
        return:
            - Tuple: dos valores randômicos da GPU.
    '''
    gpu_uso = round(random.uniform(40.0, 90.0), 1)
    gpu_temperatura = round(random.uniform(45.0, 90.0), 1)

    return gpu_uso, gpu_temperatura
...
def coletar_dados() -> list:
    ...
    try: 
        dados=[]
        uso_simulado, temp_simulado = simular_GPU() if globais["SIMULAR_GPU"] else (None, None)
        ...
        for item in monitoramento:
            ...
            if componente == 'GPU' and globais['SIMULAR_GPU']:
                print("Dados simulados da GPU: {metrica}")
                if metrica == "%":
                    dado = uso_simulado
                else:
                    dado = temp_simulado
...
# ===================================== GPU =====================================
    ...
    #Para quando Não encontrar a física e usar a simulada. 
    if not infoGPU and globais.get('SIMULAR_GPU', False):
        print("Não a GPU detectada, iniciar captura simulada")
        componentes.append({
            "componente": "GPU",
            "marca": "Simulada",
            "numeracao": 1,
            "modelo": "Virtual-GPU-01"
        })
